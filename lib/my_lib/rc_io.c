...
static uint16_t _rc_in[8]	= { 0, 0, 0, 0, 0, 0, 0, 0};
static uint16_t _rc_out[8]	= { 0x4AB5, 0x4AB5, 0x4AB5, 0x4AB5, 0x4AB5, 0x4AB5, 0x4AB5, 0x4AB5};

static void Init_rcin_port(uint8_t ch);
static void Init_rcout_port(uint8_t ch);
	
static uint16_t _pwm_period = (uint16_t)(INTERRUPT_FREQ / PWM_FREQ);

/* PWM Input port setting */
static void Init_rcin_port(uint8_t ch){
	
	switch(ch){
		case 0:
		//Ch.1 BT0 TIOB00_0
		FM3_GPIO->PFR4_f.P9 = 1; //P49 is PWM Input
		...
		case 1:
		//Ch.2 BT1 TIOB01_0
		FM3_GPIO->PFR3_f.P1 = 1; //P31 is PWM Input
		...
		case 2:
		//Ch.3 BT2 TIOB02_1
		FM3_GPIO->PFR3_f.P2 = 1; //P32 is PWM Input
		...
		case 3:
		//Ch.4 BT3 TIOB03_1
		FM3_GPIO->PFR3_f.P3 = 1; //P33 is PWM Input
...
/* PWM Output port setting */
static void Init_rcout_port(uint8_t ch){
	
	switch(ch){
		case 0:
		//Ch.1 BT8 TIOA08_0
		FM3_GPIO->PFRA_f.P0 = 1; //PA0 is PWM Output
		...
		case 1:
		//Ch.2 BT9 TIOA09_0
		FM3_GPIO->PFRA_f.P1 = 1; //PA1 is PWM Output
		...
		case 2:
		//Ch.3 BT10 TIOA10_0
		FM3_GPIO->PFRB_f.P2 = 1; //PB2 is PWM Output
		...
		case 3:
		//Ch.4 BT11 TIOA11_0
		FM3_GPIO->PFRA_f.P3 = 1; //PA3 is PWM Output
...
void rcin_enable(uint8_t ch){
	...
	//Timer setting
	rcin->TMCR_f.CTEN = 0; // Timer stop
	rcin->TMCR_f.FMD = 0x04; // select 16bit PWC Timer
	rcin->TMCR_f.T32 = 0;
	rcin->TMCR_f.CKS0 = 0; // select count clock prescaler
	rcin->TMCR_f.CKS1 = 1; // 72MHz/16 = 4.5MHz
	rcin->TMCR_f.CKS2 = 0; //
	rcin->TMCR2_f.CKS3 = 0;
#ifndef USE_PPM
	rcin->TMCR_f.EGS0 = 0; // measure High Pulse Width 
	rcin->TMCR_f.EGS1 = 0;
	rcin->TMCR_f.EGS2 = 0;
#else
	rcin->TMCR_f.EGS0 = 1; // measure Pulse Down Edge
	rcin->TMCR_f.EGS1 = 0;
	rcin->TMCR_f.EGS2 = 0;
#endif
	rcin->TMCR_f.MDSE = 0;
	
	rcin->STC_f.EDIE = 1; // Enable Measure 
	rcin->STC_f.OVIE = 0; // Disable Overflow interrupt
	
	NVIC_EnableIRQ(BTIM0_7_IRQn); 
	rcin->TMCR_f.CTEN = 1; //Start Down Counter
...
void rcout_enable(uint8_t ch){
	...
	//RCサーボ用
	rcout->PCSR = _pwm_period; //PWM周期設定=0.01sec周期
	rcout->PDUT = 1520 * TICK_PER_USEC;  //デューティ比(オフ幅）　
	//center 1.5msec = 4AB5
	//max 2.0msec = 4650
	//min 1.0msec = 4F1A
...
void rc_write(uint8_t ch, uint16_t pulse_usec){
	...
	//PWM出力はパルスのOFF幅指定
	duty = _pwm_period - (uint16_t)(pulse * TICK_PER_USEC);
...
uint16_t rcout_read(uint8_t ch){
	return (uint16_t)((_pwm_period - _rc_out[ch]) * USEC_PER_TICK);
}
...
void BT0_7_IRQHandler(void){
	...
	if(FM3_BT0_PWC->STC_f.EDIR == 1){
		...
		if(invalid == 1){
			//1100~1940に変換
			if(count >= 0) 	_rc_in[count] = (uint16_t)(pulse * USEC_PER_TICK);
			count ++;
...
#else
		_rc_in[0] = (uint16_t)(pulse * USEC_PER_TICK);
#endif
		
	}
	
	if(FM3_BT1_PWC->STC_f.EDIR == 1){ pulse = FM3_BT1_PWC->DTBF;	_rc_in[1] = (uint16_t)(pulse * USEC_PER_TICK);}
	if(FM3_BT2_PWC->STC_f.EDIR == 1){ pulse = FM3_BT2_PWC->DTBF;	_rc_in[2] = (uint16_t)(pulse * USEC_PER_TICK);}
	if(FM3_BT3_PWC->STC_f.EDIR == 1){ pulse = FM3_BT3_PWC->DTBF;	_rc_in[3] = (uint16_t)(pulse * USEC_PER_TICK);}
	if(FM3_BT4_PWC->STC_f.EDIR == 1){ pulse = FM3_BT4_PWC->DTBF;	_rc_in[4] = (uint16_t)(pulse * USEC_PER_TICK);}
	if(FM3_BT5_PWC->STC_f.EDIR == 1){ pulse = FM3_BT5_PWC->DTBF;	_rc_in[5] = (uint16_t)(pulse * USEC_PER_TICK);}
	if(FM3_BT6_PWC->STC_f.EDIR == 1){ pulse = FM3_BT6_PWC->DTBF;	_rc_in[6] = (uint16_t)(pulse * USEC_PER_TICK);}
	if(FM3_BT7_PWC->STC_f.EDIR == 1){ pulse = FM3_BT7_PWC->DTBF;	_rc_in[7] = (uint16_t)(pulse * USEC_PER_TICK);}
}
